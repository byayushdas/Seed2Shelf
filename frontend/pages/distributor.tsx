    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-500 mt-1">MahaTrans Logistics. Connected via 0x7890...abcd</p>
        </div>
               ...
               <div className="space-y-4">
                 {incomingBatches.map(batch => (
                   <div key={batch.id} className="p-4 border border-gray-100 rounded-lg hover:bg-gray-50 transition-colors">
                     ...
                     <p className="text-xs text-gray-500">From Processor: {batch.processor?.name} (Location: Pune Hub)</p>
                   </div>
                 ))}
                 {incomingBatches.length === 0 && (
                   <div className="text-center py-8 text-gray-500">No batches waiting for transport.</div>
                 )}
               </div>
             )}
           </Card>
              ...
              {selectedBatch ? (
                <form onSubmit={handleProcess} className="space-y-4">
                  <div className="bg-agri-green-100/50 p-4 rounded-lg mb-4">
                    ...
                    <p className="text-xs text-gray-600">Log vehicle conditions before signing handover to Retailer.</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Logistics Remarks</label>
                    <textarea required value={remarks} onChange={e => setRemarks(e.target.value)} rows={3} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-agri-green focus:border-agri-green" placeholder="e.g. Transported via Cold Chain Truck MH12AB1234. Temp maintained at 4°C." />
                  </div>
                  <div>
