    ...
    setPaymentError(null);
      ...
      try {
        const orderItems = cartItems.map(item => ({
          ...
          sellerId: item.harvestItem.farmerId, // Using the farmerId directly from frontend state if we had it, but we only have crop ID.
          // Wait, the API orders route uses `cropId` and `sellerId`. 
          // Let me map it based on `item.harvestItem.id`
          quantity: item.selectedQuantity,
          pricePerKg: item.harvestItem.pricePerUnit || parseFloat(item.harvestItem.pricePerKg.replace(/[^0-9.]/g, ''))
        }));

        const res = await orderService.distributorApi.placeOrder(orderItems);
