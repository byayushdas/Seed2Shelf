...
import { Sprout, CheckCircle, ShieldCheck, Tag, HelpCircle, ArrowRight, Package } from 'lucide-react';
...
export default function TraceBatch() {
  ...
  useEffect(() => {
    if (batchId) {
      setIsLoading(true);
      traceService.getTraceabilityData(batchId as string)
        .then(res => {
  ...
  if (!traceData.length) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4 text-center">
        ...
        <p className="text-gray-500 mb-6 max-w-md">We couldn't find this item on the Seed2Shelf network. It may be invalid or not yet processed.</p>
  ...
  const getIconForStage = (stage: string) => {
    switch (stage) {
      case "Farming": return <Sprout className="w-4 h-4 text-agri-green" />;
      case "Processing": return <Package className="w-4 h-4 text-agri-green" />;
      default: return <CheckCircle className="w-4 h-4 text-agri-green" />;
    }
  };
  ...
  return (
    <>
      <Head>
        <title>Verified Origin | Seed2Shelf Trace</title>
      </Head>
      ...
      <div className="bg-agri-green-900 text-white pb-24 pt-12 relative overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto px-4 relative z-10 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl font-extrabold mb-4">Traceability Report</h1>
          <p className="text-agri-green-100 text-lg">ID #{batchId}</p>
        </div>
      </div>
        ...
        <Card className="mb-8 shadow-2xl border-0 !p-8 relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            <div>
              ...
              <h2 className="text-2xl font-bold text-gray-900">Verified Journey</h2>
              <p className="text-gray-600 mt-1 flex items-center gap-1">
                <span className="text-xs">✅</span> Traced from Source
              </p>
            </div>
            ...
            <div className="bg-green-50 rounded-xl p-4 border border-green-100 flex-shrink-0 w-full sm:w-auto text-center sm:text-right">
              <p className="text-sm font-medium text-green-800 mb-1">Stages Logged:</p>
              <p className="text-3xl font-extrabold text-agri-green">{traceData.length}<span className="text-lg font-medium text-green-700"> steps</span></p>
            </div>
          </div>
        </Card>
            ...
            <div className="relative" key={index}>
              ...
              <div>
                <div className="flex flex-wrap items-baseline justify-between mb-1">
                  <h4 className="text-lg font-bold text-gray-900">{index + 1}. {stage.title}</h4>
                  <div className="text-xs text-agri-green-900 font-mono bg-agri-green-100 px-2 pl-3 py-1 rounded-full" title={stage.txHash}>
                    {stage.txHash.length > 20 ? `${stage.txHash.slice(0, 10)}...` : stage.txHash}
                  </div>
                </div>
                <p className="text-sm text-gray-500 mb-3">{stage.date}</p>
                <div className="bg-white p-4 rounded-lg border border-gray-100 shadow-sm">
                  <p className="font-medium text-gray-900 mb-2">{stage.actor}</p>
                  <p className="text-sm text-gray-600 mb-3 pb-3 border-b border-gray-50 italic">
                    "{stage.details}"
                  </p>
                  <div className="flex justify-between items-center text-sm border-t border-gray-50 pt-2 mt-2">
                    <span className="text-gray-500">Location:</span>
                    <span className="font-bold text-agri-green">{stage.location}</span>
                  </div>
                </div>
              </div>
            </div>
