...
export interface InventoryItem {
  ...
  itemType: "RETAILED" | "RAW"; // Retailed Product vs Purchased Raw Crop
  productName: string;
  category: string;
  quantity: string;
  pricePerUnit: string;
  ...
  productImage?: string;
  qrCodeUrl: string;
  processingStatus?: "Available for Retail" | "Sent for Retail" | "Fully Retailed";
...
export default function RetailHubPage() {
  ...
  useEffect(() => {
    ...
    const fetchInventory = async () => {
      try {
        const res = await fetch(`${BACKEND_URL}/api/v1/retailer/inventory?userId=${retailerId}`);
        if (res.ok) {
          ...
          if (json.success && Array.isArray(json.data)) {
            const mapped = json.data.map((item: any) => ({
              ...
              status: item.status,
              parentRawBatchId: item.parentDistBatchId,
        ...
        console.error("Failed to fetch retailer inventory:", err);
      }
    };
    fetchInventory();
  }, [retailerId]);
  ...
  // Filter state for My Inventory (Primary Tabs: Retailed Products, Purchased Harvests, History)
  const [inventoryFilter, setInventoryFilter] = useState<"RETAILED" | "RAW" | "HISTORY">("RETAILED");

  // History Tab Category Filter (Retailed Products vs Purchased Harvests)
  const [historyCategoryFilter, setHistoryCategoryFilter] = useState<"RETAILED_DISPATCHED" | "RAW_COMPLETED">("RETAILED_DISPATCHED");
  ...
  const handleRegisterProduct = (e: React.FormEvent) => {
    ...
    const newBatchId = `RET-2026-${randomDigits}`;
    ...
    const processSave = async () => {
      try {
        const res = await fetch(`${BACKEND_URL}/api/v1/retailer/inventory`, {
          method: "POST",
          ...
          body: JSON.stringify({
            ...
            quantity: quantity,
            pricePerUnit: price,
            parentDistBatchId: parentRawBatchIds.join(", "),
        ...
        console.error("Error saving retailed product:", err);
      }
    };
  ...
  const handleDeleteItem = async (id: string) => {
    if (confirm("Are you sure you want to remove this item from your inventory?")) {
      try {
        const res = await fetch(`${BACKEND_URL}/api/v1/retailer/inventory/${id}`, { method: "DELETE" });
        if (res.ok) {
    ...
    setInventory((prev) =>
      prev.map((item) => {
        if (item.id === itemId) {
          ...
          const newStatus = newRemainingNum === 0 ? "Fully Retailed" : "Sent for Retail";
      ...
      setInventory((prev) =>
        prev.map((item) => {
          if (item.id === id) {
            ...
            return {
              ...item,
              processingStatus: "Available for Retail",
  ...
  const filteredInventory = inventory.filter((item) => {
    if (inventoryFilter === "RETAILED") return item.itemType === "RETAILED";
    ...
    <div className="min-h-screen text-stone-100 font-sans pb-24 pt-6 px-4 sm:px-6 lg:px-8 relative z-20">
      ...
      <div className="max-w-6xl mx-auto space-y-7">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-7">
          ...
          {/* =========================================================================
              LOG NEW RETAILED ITEM FORM (LEFT CARD - 6 COLS)
             ========================================================================= */}
          <div className="lg:col-span-6 bg-stone-900/90 border border-stone-800 rounded-3xl p-6 sm:p-7 shadow-xl space-y-5">
            <div className="pb-3 border-b border-stone-800">
              <h2 className="text-base font-extrabold text-emerald-400 flex items-center gap-2">
                <PlusCircle className="w-5 h-5" /> Log New Retailed Item
              </h2>
            </div>
            ...
            <form onSubmit={handleRegisterProduct} className="space-y-4 text-xs">
              ...
              {/* Retailed Product Photo Upload */}
              <div>
                <label className="text-stone-400 font-bold uppercase text-[10px] tracking-wider block mb-1.5">
                  Retailed Product Photo *
                </label>
                <input
                  type="file"
                  accept="image/*"
          ...
          <div className="lg:col-span-6 bg-stone-900/90 border border-stone-800 rounded-3xl p-6 sm:p-7 shadow-xl space-y-5">
            <div className="pb-3 border-b border-stone-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              {/* PRIMARY FILTER TABS: Retailed Products vs Purchased Harvests vs History */}
            ...
            {inventoryFilter === "RAW" ? (
              ...
              <div className="space-y-3">
                {inventory.filter(i => i.itemType === "RAW" && i.processingStatus !== "Fully Retailed" && parseQuantityAndUnit(i.remainingStock || i.quantity).num > 0).length === 0 ? (
                  <div className="p-8 text-center text-stone-400 text-xs">
                    No active purchased farmer crop harvests available. Completed batches move to History.
                  </div>
                ) : (
                  inventory.filter(i => i.itemType === "RAW" && i.processingStatus !== "Fully Retailed" && parseQuantityAndUnit(i.remainingStock || i.quantity).num > 0).map((raw) => {
                    const imgUrl = getProductImage(raw);
                    const isSent = raw.processingStatus === "Sent for Retail";
                    const isFullyProcessed = raw.processingStatus === "Fully Retailed";
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px] pt-0.5">
                          ...
                          <div className="p-2.5 bg-stone-900/60 rounded-xl border border-stone-800/60">
                            <span className="text-[10px] text-stone-500 font-bold uppercase block mb-0.5">In Retail</span>
                        ...
                        <div className="grid grid-cols-3 gap-2 text-[11px] pt-1 border-t border-stone-800/60">
                          <div className="p-2 bg-stone-900/40 rounded-xl border border-stone-800/40">
                            <span className="text-[9px] text-stone-500 font-bold uppercase block">Retail Status</span>
                            <span className={`font-bold text-[10px] ${isSent ? "text-amber-400" : "text-emerald-400"}`}>
                              {raw.processingStatus || "Available for Retail"}
                            </span>
                          </div>

                          <div className="p-2 bg-stone-900/40 rounded-xl border border-stone-800/40">
                            <span className="text-[9px] text-stone-500 font-bold uppercase block">Last Retail Date</span>
              ...
              /* =========================================================================
                 TAB 2: RETAILED PRODUCTS (ACTIVE REGISTERED PRODUCTS)
                 ========================================================================= */
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {inventory.filter(i => i.itemType === "RETAILED" && i.status !== "Dispatched" && i.status !== "Archived").length === 0 ? (
                  <div className="col-span-full p-8 text-center text-stone-400 text-xs">
                    No active processed product items registered yet.
                  </div>
                ) : (
                  inventory.filter(i => i.itemType === "RETAILED" && i.status !== "Dispatched" && i.status !== "Archived").map((item) => {
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-stone-950 p-2.5 rounded-2xl border border-stone-800 text-[11px] font-bold gap-2">
                  <span className="text-stone-400 text-xs font-bold pl-1">Retailer History Logs</span>
                  <div className="flex items-center gap-1 bg-stone-900 p-1 rounded-xl border border-stone-800/80 text-[10px]">
                    <button
                      onClick={() => setHistoryCategoryFilter("RETAILED_DISPATCHED")}
                      className={`px-3 py-1.5 rounded-lg transition cursor-pointer flex items-center gap-1 ${
                        historyCategoryFilter === "RETAILED_DISPATCHED"
                          ? "bg-emerald-600 text-white font-extrabold shadow-sm"
                          : "text-stone-400 hover:text-stone-200"
                      }`}
                    >
                ...
                {inventory.filter(i => {
                  if (historyCategoryFilter === "RAW_COMPLETED") {
                    return i.itemType === "RAW" && (i.processingStatus === "Fully Retailed" || parseQuantityAndUnit(i.remainingStock).num === 0);
                  }
                  return i.itemType === "RETAILED" && (i.status === "Dispatched" || i.status === "Archived");
                }).length === 0 ? (
                  <div className="p-8 text-center text-stone-400 text-xs">
                    {historyCategoryFilter === "RAW_COMPLETED"
            ...
            <div className="bg-stone-900 border border-stone-800 rounded-3xl p-6 sm:p-7 max-w-lg w-full space-y-5 shadow-2xl relative">
              ...
              <div className="p-4 bg-stone-950 border border-stone-800 rounded-2xl space-y-2 text-xs">
                ...
                <div className="grid grid-cols-2 gap-2 pt-2 border-t border-stone-800/80 text-[11px]">
                  ...
                  <div>
                    <span className="text-stone-500 text-[10px] block">Currently In Retail</span>
                ...
                <div className="p-3.5 bg-stone-950/80 rounded-2xl border border-stone-800 space-y-1.5 text-[11px]">
                  ...
                  <div className="flex justify-between items-center text-stone-300 pt-1 border-t border-stone-800/60">
                    ...
                    <span className={`font-extrabold text-[10px] px-2 py-0.5 rounded-full border ${
                      ...
                      {projectedRemaining === 0 ? "Fully Retailed" : "Sent for Retail"}
                    </span>
                  </div>
                </div>
