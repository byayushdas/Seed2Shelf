...
export default function ProcessorInvoicesPage() {
  ...
  // Processor Accounting Invoices
  const mockProcessorInvoices: any[] = [];

  const filteredInvoices = mockProcessorInvoices.filter((inv) => {
  ...
  return (
    <div className="min-h-screen text-stone-100 font-sans pb-24 pt-6 px-4 sm:px-6 lg:px-8 relative z-20">
      <Head>
        <title>Wallet Invoices | Seed2Shelf Processor</title>
        <meta name="description" content="Processor sales and raw material purchase tax invoices" />
      </Head>
      ...
      <div className="max-w-5xl mx-auto space-y-7">
        ...
        {/* =========================================================================
            PAGE HEADER (MATCHING FARMER WALLET INVOICES HEADER STYLE 1-TO-1)
           ========================================================================= */}
        ...
        <div className="space-y-3">
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
            ...
            <div className="relative flex-1 max-w-md">
              ...
              <input
                type="text"
                placeholder="Search by Invoice #, Order #, Batch, Supplier, or Buyer..."
                value={searchQuery}
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between px-1 h-6">
            <h2 className="text-xs font-bold text-stone-400 uppercase tracking-wider">
              {categoryFilter === "PURCHASE"
                ? "Purchase Invoices (Raw Crop Purchases)"
                : categoryFilter === "SALES"
                ? "Sales Invoices (Processed Product Sales)"
                : "Trade Invoices"}
            </h2>
          ...
          {filteredInvoices.length === 0 ? (
            <div className="bg-stone-900/90 border border-stone-800 rounded-3xl p-10 text-center text-xs text-stone-400 space-y-2">
              ...
              <p className="text-stone-500">Try searching with a different Invoice Number, Batch ID, or Partner Name.</p>
            </div>
          ) : (
                ...
                <table className="w-full text-left border-collapse text-xs sm:text-sm">
                  <thead>
                    <tr className="border-b border-stone-800 text-stone-400 text-[11px] font-bold uppercase tracking-wider">
                      ...
                      <th className="pb-3.5 align-middle">Item & Partner Details</th>
                      ...
                      >
                        <td className="py-4 pl-2 font-bold text-white align-middle">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <span className="font-mono text-xs block">{inv.id}</span>
                              <span className="text-[10px] text-stone-500 font-mono">
                                {inv.category === "SALES" ? "Sales" : "Purchase"}
                              </span>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="py-4 text-stone-200 font-medium align-middle">
                          <div>
                            ...
                            <span className="text-[11px] text-stone-400">
                              {inv.category === "SALES" ? "Buyer: " : "Supplier: "}
                              <strong className="text-stone-300">
                                {inv.category === "SALES" ? inv.buyer : inv.supplier}
                              </strong>
                            </span>
                          </div>
                        </td>
