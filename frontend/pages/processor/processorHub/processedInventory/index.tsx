...
export default function ProductionHubPage() {
  ...
  // Filter state for My Inventory (Primary Tabs: Processed Products, Purchased Harvests, History)
  const [inventoryFilter, setInventoryFilter] = useState<"PROCESSED" | "RAW" | "HISTORY">("PROCESSED");

  // History Tab Category Filter (Processed Products vs Purchased Harvests)
  const [historyCategoryFilter, setHistoryCategoryFilter] = useState<"PROCESSED_DISPATCHED" | "RAW_COMPLETED">("PROCESSED_DISPATCHED");
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-7">
          ...
          <div className="lg:col-span-6 bg-stone-900/90 border border-stone-800 rounded-3xl p-6 sm:p-7 shadow-xl space-y-5">
            <div className="pb-3 border-b border-stone-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              {/* PRIMARY FILTER TABS: Processed Products vs Purchased Harvests vs History */}
            ...
            {inventoryFilter === "RAW" ? (
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-stone-950 p-2.5 rounded-2xl border border-stone-800 text-[11px] font-bold gap-2">
                  <span className="text-stone-400 text-xs font-bold pl-1">Processor History Logs</span>
                  <div className="flex items-center gap-1 bg-stone-900 p-1 rounded-xl border border-stone-800/80 text-[10px]">
                    <button
                      onClick={() => setHistoryCategoryFilter("PROCESSED_DISPATCHED")}
                      className={`px-3 py-1.5 rounded-lg transition cursor-pointer flex items-center gap-1 ${
                        historyCategoryFilter === "PROCESSED_DISPATCHED"
                          ? "bg-emerald-600 text-white font-extrabold shadow-sm"
                          : "text-stone-400 hover:text-stone-200"
                      }`}
                    >
                ...
                {inventory.filter(i => {
                  if (historyCategoryFilter === "RAW_COMPLETED") {
                    return i.itemType === "RAW" && (i.processingStatus === "Fully Processed" || parseQuantityAndUnit(i.remainingStock).num === 0);
                  }
                  return i.itemType === "PROCESSED" && (i.status === "Dispatched" || i.status === "Archived");
                }).length === 0 ? (
                  <div className="p-8 text-center text-stone-400 text-xs">
                    {historyCategoryFilter === "RAW_COMPLETED"
